...
-- choosing the that will be used
...
--Select dea.continent, dea.location, dea.date, dea.population, vac.new_vaccinations, sum(convert (int ,vac.new_vaccinations)) over (partition by dea.location order by dea.location, dea.date) as countingVac
--from dbo.CovidDeaths$ as dea
--join dbo.CovidVaccinations$ as vac
--on dea.location = vac.location
--and dea.date = vac.date
